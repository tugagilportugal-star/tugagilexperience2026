...
export const Program: React.FC<ProgramProps> = ({ onOpenTicketModal }) => {
  ...
  const morningHighlights: AgendaItem[] = [
    ...
    },
    {
      ...
      title: "Como Liderar Equipas Autónomas sem Perder o Alinhamento Estratégico (título sugerido)",
      speaker: "A Confirmar",
    ...
    },
    {
      ...
      title: "Práticas Modernas de Engenharia de Software no Contexto Ágil (título sugerido)",
      speaker: "A Confirmar",
  ...
  const afternoonHighlights: AgendaItem[] = [
    {
      time: "14h10 - 16h30",
      category: "Comunidade",
      title: "7 Talks Curtas da Comunidade Ágil de Portugal (título sugerido)",
    ...
    },
    {
      ...
      title: "Fishbowl: O Futuro da Agilidade em Portugal (título sugerido)",
      speaker: "Participação Aberta ao Público",
      role: "Painel de Discussão Dinâmico",
      avatar: "https://images.unsplash.com/photo-1515187029135-18ee286d815b?auto=format&fit=crop&w=200&q=80"
    },
    {
      time: "18h00 - 19h00",
      category: "Encerramento",
      title: "Keynote de Encerramento & Considerações Finais (título sugerido)",
      speaker: "A Confirmar",
  ...
  return (
    <section id="program" className="bg-zinc-950 text-white py-24 border-t border-zinc-900/60">
      <div className="max-w-4xl mx-auto px-6">
        ...
        {/* Lista de Fluxo Vertical (Tipografia Reforçada) */}
        <div className="relative border-l border-zinc-800 ml-4 pl-6 md:pl-10 space-y-12">
          {(activeTab === 'morning' ? morningHighlights : afternoonHighlights).map((item, index) => (
            <div key={index} className="relative">
              ...
              <h3 className="text-2xl md:text-3xl font-black text-white font-sora leading-tight mb-4">
                {item.title}
              </h3>
        ...
        <div className="mt-16 p-6 bg-zinc-900/30 border border-zinc-900 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-sm text-zinc-400 font-jakarta mt-1">
              Garanta o seu lugar na primeira edição do TugÁgil Experience.
            </p>
          </div>
          <button 
